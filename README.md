...
| Tenancy | `storeId` (cloud store) + `namespace` + optional `userId` | `namespace` + optional `userId` |
...
| `serverUrl` | string | `http://localhost:8000` (self-hosted) | Base URL of the backend. Cloud: the Redis Agent Memory endpoint (falls back to `AGENT_MEMORY_ENDPOINT`). Self-hosted: your server's URL. |
...
| `storeId` | string | unset | Redis Agent Memory store id. **Cloud only** (falls back to `AGENT_MEMORY_STORE_ID`). |
...
- **Existing `serverUrl`-only configs keep working unchanged.** A config with `serverUrl` set and no `storeId` (and no `AGENT_MEMORY_STORE_ID` env var) is auto-detected as `self-hosted`, exactly as before this change.
